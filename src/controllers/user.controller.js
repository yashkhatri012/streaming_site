...
const getUserchannelProfile = asyncHandler(async(req, res)=>{
    ...
    if (!username?.trim()) {
        throw new ApiError(400, "Username if missing")
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $lookup: {
                ...
                as: "subscribers" // User ko jin ne subscribe kar  rakha hai
            }
        },
        {
            $lookup: {
                ...
                as: "subscriberedTo"  //Jin channels User  subscribed kara hai
            }
        },
        { // adds the size me below fields to the final 
            $addFields:{
                subscribersCount:{
                    $size:  "$subscribers"
                },
                ...
                isSubscribed: {
                    $cond : {
                        ...
                        // $ sign here mean subscriber is a field and we can go further in it
                        // Here we are checking if subscribers document's subscriber field has the user._id
                        then: true,
                        else: false
                    }
                }
